         ...
         fetch(`/search_characters?query=${query}`)
             ...
             .then(data => {
                 ...
                 filteredData.forEach(item => {
                     ...
                     resultItem.innerHTML = `<img src="/static/images/${item.chr_name.toLowerCase().replace(/\s+/g, '_')}.png" alt="${item.chr_name}"> ${item.chr_name}`;
...
.then(data => {
 ...
 let attributesHTML = `
         <img src="/static/images/${data.character_data.chr_name.toLowerCase().replace(/\s+/g, '_')}.png" class="character-image" alt="${data.character_data.chr_name}" style="opacity: 0;">
